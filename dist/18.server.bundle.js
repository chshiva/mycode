...
/***/ function(module, exports, __webpack_require__) {
	  ...
	  function CreateRoom(props) {
	    ...
	    _this.save = function (event) {
	      ...
	      if (response.error.length > 0) {
	        ...
	        _this.props.dispatch((0, _RoomActions.SaveRoom)(_this.form)).then(function (res) {
	    ...
	    _this.setresponse = function (response) {
	      if (response.status) {
	        _reactRouter.browserHistory.push('/admin/room/adduser/' + response.data._id);
	        ...
	        if (_this.state.role == _roles.Roles.Lmsadmin || _this.state.role == _roles.Roles.Instructor || _this.state.role == _roles.Roles.Student) {
	          var ShowErrMsg = '';
	          var RegEx = new RegExp('Room', 'g');
	          ShowErrMsg = errMsg.replace(RegEx, 'Course');
	          _this.refs.room_container.error(ShowErrMsg + ' ', '');
	        } else {
	          _this.refs.room_container.error(errMsg + ' ', '');
	        }
	      }
	    };
	    ...
	    _this.submenu = _Validator2.default.activeSubMenu(_RoomMenu.roomNewSubMenu, "lnkNewRoom");
	    _this.mainmenu = _RoomMenu.roomNewMainMenu;
	...
	exports.default = (0, _reactRedux.connect)(mapStateToProps)(CreateRoom);
