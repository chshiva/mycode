...
/***/ function(module, exports, __webpack_require__) {
	...
	var ListUser = function (_Component) {
	  ...
	  function ListUser(props) {
	    ...
	    _this.fileFilter = function (e) {
	      e.preventDefault();
	      _this.setState({ userStatus: e.target.value }, function () {
	        this.getData({
	    ...
	    _this.showCompanyId = function (row) {
	      ...
	      if (row.profile.companyid == null) {
	        ...
	        if (_roles.Roles.Superadmin == _this.props.loggedInData.data.role) {
	          var rowId = row.profile.companyid._id;
	          var viewlink = "/admin/corporate/view/" + rowId;
	          return _jsx(_reactRouter.Link, {
	            className: _Admin2.default.removeStyle,
	            to: viewlink
	          }, void 0, _jsx('div', {}, void 0, row.profile.companyid.businessId));
	        } else {
	    ...
	    _this.importUsers = function (e) {
	      ...
	      if (!file) {
	        ...
	        alertify.alert(_this.props.intl.messages.warning, _this.props.intl.messages.topic_file_alert, function () {}).setting({ 'label': _this.props.intl.messages.ok });
	        return;
	      }
	      //console.log(file)
	    ...
	    _this.submenu = _Validator2.default.activeSubMenu(_UserMenu.userListSubMenu, "lnkUsers");
	    _this.mainmenu = _UserMenu.userListMainMenu;
	    _this.getData = _this.getData.bind(_this);
	
	    _this.currentPage = 1;
	    _this.itemsPerPage = 5;
	
	    _this.userView = _this.userView.bind(_this);
	    _this.mainmenu.menus[0].action = _this.clearError.bind(_this);
	    _this.mainmenu.menus[1].action = _this.exportUsers.bind(_this);
	    _this.mainmenu.menus[2].action = _this.importUsers.bind(_this);
	  ...
	  _createClass(ListUser, [{
	    ...
	    value: function getData(pageParam) {
	      ...
	      this.props.dispatch((0, _UsersActions.UserList)(pageParam, pageParam.currentPage)).then(function (res) {
	    ...
	    key: 'userView',
	    value: function userView(row) {
	      var rowId = row._id;
	      var viewlink = "/admin/users/view/" + rowId;
	      return _jsx(_reactRouter.Link, {
	        id: 'viewUser',
	        to: viewlink
	      }, void 0, _ref2);
	    ...
	    value: function combineName(row) {
	      ...
	      return _jsx(_reactRouter.Link, {
	        className: _Admin2.default.removeStyle,
	        to: viewlink
	      }, void 0, _jsx('div', {}, void 0, row.firstname, ' ', row.lastname));
	    }
	  }, {
	    key: 'showRole',
	    ...
	    value: function showFormattedDate(row) {
	      return _jsx('div', {}, void 0, (0, _moment2.default)(row.dateAdded).format('DD-MM-YYYY'));
	    }
	  }, {
	    key: 'showEmail',
	    ...
	    value: function exportUsers() {
	      ...
	      if (this.state.searchValue != undefined && this.state.searchKeyword != '') {
	        ...
	        this.props.dispatch((0, _UsersActions.exportUsersRequest)(obj)).then(function (res) {
	          return _this3.exportData(res);
	        });
	      } else {
	        var _obj = {
	          userStatus: this.state.userStatus ? this.state.userStatus : 'Active'
	        };
	        this.props.dispatch((0, _UsersActions.exportUsersRequest)(_obj)).then(function (res) {
	          return _this3.exportData(res);
	        });
	      }
	    }
	        ...
	        complete: function complete(err, res) {
	          ...
	          if (err) {
	            ...
	            if (_data.status == false) {
	              self.refs.user_container.error(_data.error);
	            } else if (_data.data.length <= 0) {
	              self.refs.user_container.error("Please import valid data");
	            } else {
	              var importData = _data.data;
	              var _userData = {};
	              _userData['uid'] = self.props.loggedInData.data._id;
	              _userData['currentPage'] = self.currentPage, _userData['totalItems'] = 0, _userData['itemsPerPage'] = self.itemsPerPage, _userData['searchKeyword'] = self.state.searchValue;
	              self.props.dispatch((0, _UsersActions.importUsersRequest)({ importData: importData, userData: _userData }, self.currentPage)).then(function (res) {
	                return self.importData(res);
	              });
	    ...
	    key: 'importData',
	    value: function importData(response) {
	    ...
	    value: function render() {
	      ...
	      var objDisp = [{ title: _ref4, type: "function", callback: this.combineName, sort: true, dbName: "firstname" }, { fieldName: "email", title: _ref5, type: "function", sort: true, callback: this.showEmail, dbName: "email" }, { title: _ref6, type: "function", callback: this.showRole, sort: true, dbName: "role" }, { title: _ref7, type: "function", callback: this.showCompanyId }, { title: _ref8, type: "function", callback: this.showFormattedDate, sort: true, dbName: "modifiedAt" }, { title: _ref9, type: "function", callback: this.userView }];
	
	      var filter = [{ type: 'search', id: "userSearch", selectedfilter: this.searchFilter }, { type: 'dropdown', id: "userType", data: [['Active', 'active'], ['Registered', 'registered'], ['Deleted', 'deleted'], ['Blocked', 'blocked'], ['Guest', 'guest']], selectedfilter: this.fileFilter, value: this.state.userStatus }];
	...
	exports.default = (0, _reactRedux.connect)(mapStateToProps)(ListUser);
